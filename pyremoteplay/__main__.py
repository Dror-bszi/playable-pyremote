...
class CLIInstance:
    ...
    def run(self, stdscr):
        """Run CLI Instance."""
        # Start Hardware Producer subprocess
        ...
        # PipeReader is now handling all pipe communication in a separate thread
        # The controller will receive commands from both Hardware Producer and Vision Sensor
        # Keep the main thread alive while the session is running
        try:
            while self._device.session.is_ready:
                sleep(1)
